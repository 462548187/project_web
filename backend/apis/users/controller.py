...
async def all_users(page_no: int, page_size: int, search_username: str = '', db: Session = Depends(get_db)):
    # 分页查询,前端需要传递页数,和每页多少个 数据分页与list切片格式保持一致
    if search_username:
...
async def create_user(user: NewUser, request: Request, db: Session = Depends(get_db)):
    ...
    db.commit()
    # 调用数据修改记录器
    Record.create_operate_record(new_object=new_record, username=User.username, ip=request.client.host)
...
async def update_user(request: Request, modify_user: ModifyUser, db: Session = Depends(get_db)):
    ...
    if user:
        ...
        db.commit()
        # 调用数据修改记录器
        Record.create_operate_record(old_object=old_user, new_object=new_user, username=User.username,
